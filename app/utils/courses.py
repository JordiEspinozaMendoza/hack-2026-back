...
def stream_generate_course(
    transcript_name: str, course_id: str, title: str, description: str
    ...
    try:
        ...
        if redis_client:
            print(f"Checking Redis cache for enhanced course with ID: {course_id}...")
            cached = redis_client.get(f"course:{course_id}")
            if cached:
                yield "Found enhanced course in cache! Loading...\n"
                return json.loads(cached)
            else:
                yield "No cache found for course. Generating from transcript...\n"
                print("No cache found for course. Generating from transcript...")
